...
def inclination(code="005930"):
    ...
    s = 0
    i = []
    i.append(int(output_list[0]['stck_prpr']) - int(output_list[1]['stck_prpr']))
    i.append(int(output_list[1]['stck_prpr']) - int(output_list[2]['stck_prpr']))
    i.append(int(output_list[2]['stck_prpr']) - int(output_list[3]['stck_prpr']))
    i.append(int(output_list[3]['stck_prpr']) - int(output_list[4]['stck_prpr']))
    i.append(int(output_list[4]['stck_prpr']) - int(output_list[5]['stck_prpr']))
    s = sum(i)
    result = False
    if 0<= i[0] <= 1 and i[1] <= 0 and s <= 0:
        result = True
    return result
...
try:
    ...
    #day_max_percent = 1.02
    #day_min_percent = 0.97
    buy_amount = total_cash * buy_percent  # 종목별 주문 금액 계산
    #day_max_amount = total_amount * day_max_percent
    #day_min_amount = total_amount * day_min_percent
...
#________한 번 쓸 시간 값들_______________________________________________________
    ...
    holiday_check, week = check_open_mkt(date)
#_________________________________________________________________________________
    if holiday_check == "N" or week < 2:  # 토요일이나 일요일이면 자동 종료
        send_message("휴장일이므로 프로그램을 종료합니다.")
                    ...
                    if len(symbol_list) < 1:#첫 종목 리스트 조회
                        ...
                        for sym in symbol_list:
                            ...
                            if inclination(sym):
                                ...
                                if rt[3] * 0.35 < rt[0] < rt[3] * 0.65 and rt[1] < 0 and rt[2] > 0 and rt[1] * -0.8 > rt[2]:
