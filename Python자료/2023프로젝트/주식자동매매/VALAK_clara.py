...
def get_asking_price_rt(code="005930"):
    ...
    for i in range(1,6):
        ...
        sum_a += int(res.json()['output1'][a_str_idx])
        sum_b += int(res.json()['output1'][b_str_idx])
    rt = float(sum_a) / float(sum_b)
    total_rt = float(res.json()['output1']['total_askp_rsqn']) / float(res.json()['output1']['total_bidp_rsqn'])
    a_ti = float(res.json()['output1']['total_askp_rsqn_icdc'])
    b_ti = float(res.json()['output1']['total_bidp_rsqn_icdc'])
    return [rt, total_rt, a_ti, b_ti]
def get_rt_dic():
    """주식 잔고 평가 손익율 조회"""
...
#   day_max_percent = 0.02
    buy_amount = total_cash * buy_percent  # 종목별 주문 금액 계산
#    day_max_amount = total_cash * day_max_percent
    soldout = False 
...
#________한 번 쓸 시간 값들_______________________________________________________
    ...
    t_9 = t.replace(hour=9, minute=0, second=0, microsecond=0)
    t_start = t.replace(hour=9, minute=5, second=0, microsecond=0)
    t_sell = t.replace(hour=15, minute=15, second=0, microsecond=0)
    t_exit = t.replace(hour=15, minute=20, second=0,microsecond=0)
        ...
        while True:
            ...
            if t_start < t_now < t_sell :  # AM 09:05 ~ PM 03:15 : 매수
                ...
                if len(symbol_list) < 1:#첫 종목 리스트 조회
                    ...
                    time.sleep(20)
                elif len(bought_list) < target_buy_count:
                    for sym in symbol_list:
                        if sym in bought_list:
                            continue
                        current_price = get_current_price(sym)
                        rt = get_asking_price_rt(sym)
                        if rt[0] < 0.3 and rt[2] < 0 and rt[3] > 0 and rt[0] < rt[1]:
                ...
                if soldout == False:
                    stock_dict=get_stock_dic()
                    rt_dict = get_rt_dic()
                    for sym, rt in rt_dict.items():
                        r = float(rt)
                        if r >= 0.7:
                            sell(sym, stock_dict[sym])
                            send_message("1%이상 익절했습니다.ㅋㅋ")
                            ...
                            sell(sym, stock_dict[sym])
                            send_message("2.5%이하 손절했습니다.ㅠㅠ")
                            bought_list.remove(sym)
                            if len(bought_list) == 0:
                                soldout = True
                            time.sleep(1)
                if t_now.minute % 30 == 0 and t_now.second <= 10:#30분에 한번씩 리스트 초기화
                    symbol_list = set_csv_krx.set_dataframe_rate(date)
